<template>
  <div class="data-table-wrapper">
        ...
        <el-table-column type="expand">
          <template slot-scope="scope">
            ...
            >
              ...
              <template
                v-if="$scopedSlots['sub-table-actions']"
                slot="sub-table-actions"
                slot-scope="slotProps"
              >
                <slot
    ...
    <!-- 匯出預覽對話框 -->
    <export-preview-enhanced-dialog
      ...
      :has-child-items="hasSubTableItems"
...
import EnhancedSubTable from "./EnhancedSubTable.vue";
import BatchUpdateEnhancedDialog from "./BatchUpdateEnhancedDialog.vue";
import ExportPreviewEnhancedDialog from "./ExportPreviewEnhancedDialog.vue";
...
export default {
  ...
  components: {
    EnhancedSubTable,
    BatchUpdateEnhancedDialog,
    ExportPreviewEnhancedDialog
  },
  ...
  props: {
    ...
    processorOptions: {
      type: Array,
      default: () => [
        { value: "", label: "未分配", type: "warning", icon: "el-icon-user" },
        {
          value: "客服A",
          label: "客服A",
          type: "primary",
  ...
  data() {
    return {
      ...
      // 客戶端高度
      clientHeight: 400,
      // 更新類型：'status' 表示售後案件狀態, 'processor' 表示處理人員
      updateType: "status",
      // 當前處理的狀態字段
  ...
  computed: {
    ...
    // 根據更新類型返回對應的狀態選項
    activeStatusOptions() {
      return this.updateType === "processor"
    ...
    // 是否存在子表格項目
    hasSubTableItems() {
      return this.subTableConfigs && this.subTableConfigs.length > 0;
    }
  },
  ...
  methods: {
    ...
    handleSubTableChange(index) {
      ...
      this.$emit("sub-table-change", index);
    },

    // 獲取當前激活的子表格列配置
    getActiveSubTableColumns() {
      if (!this.subTableConfigs || !this.subTableConfigs.length) {
        return [];
      }
      ...
      const index = Math.min(
        ...
        this.subTableConfigs.length - 1
      );
      return this.subTableConfigs[index].columns || [];
    },
    ...
    handleBatchUpdateOrder() {
      ...
      // 打開對話框，並傳遞狀態類型
      this.updateType = "status"; // 標記當前是更新售後案件狀態
      this.currentStatusField = "status";
    ...
    handleBatchUpdatePayment() {
      ...
      // 打開對話框，並傳遞狀態類型
      this.updateType = "processor"; // 標記當前是更新處理人員
      this.currentStatusField = "processor";
      this.batchUpdateDialogVisible = true;
    },
    ...
    handleUpdateStatus(updateResult) {
      ...
      updateResult.rows.forEach(row => {
        ...
        if (index !== -1) {
          ...
          if (this.updateType === "status") {
            // 如果更新的是售後案件狀態
            if (updateResult.status === "已完成") {
              // 可以添加完成案件時的額外邏輯處理
              this.data[index].completionDate = new Date();
            } else if (updateResult.status === "已取消") {
              // 添加取消案件時的額外邏輯處理
              this.data[index].cancellationReason =
                updateResult.comment || "批量取消";
            }
          }

          // 更新備註
          if (updateResult.comment) {
            this.data[index].processingRemarks = updateResult.comment;

            // 如果有最後更新時間字段，同時更新
            if (this.data[index].lastUpdateTime) {
              this.data[index].lastUpdateTime = new Date();
            }
          }
        }
      });
      ...
      if (!updateResult.exportAfterUpdate) {
        let message = `已成功更新${updateResult.rows.length}條${
          this.updateType === "status" ? "案件狀態" : "處理人員"
        }信息`;
    ...
    //   // 獲取當前激活的子表格配置
    //   const activeConfig = this.subTableConfigs[this.activeSubTableIndex];
    //   if (!activeConfig || !activeConfig.dataKey) {
    //     return [];
    //   }

    //   // 根據配置的dataKey獲取子表格數據
    //   return row[activeConfig.dataKey] || [];
    // },
    ...
    handleExportData(exportData) {
      try {
        ...
        if (result) {
          // 更新成功訊息以反映多個子表格的總數量
          let subItemsCount = 0;
          if (exportData.allSubTables && exportData.allSubTables.length) {
            exportData.allSubTables.forEach(subTable => {
              if (subTable.items) {
                subItemsCount += subTable.items.length;
              }
            });
          }
          ...
          this.$message.success(
            `成功匯出 ${exportData.selectedRows.length} 筆主項目資料${
              subItemsCount ? ` 和 ${subItemsCount} 筆子項目資料` : ""
            }`
          );
        } else {
